...
As Paul, I want canonical application statuses so that status values mean the same thing across Career System and Motorweb.
...
```bash
grep -R "US-CS-043" docs/requirements || true
```
