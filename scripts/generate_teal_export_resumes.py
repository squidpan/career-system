...
def extract_resume_body(body: str) -> str:
    marker = "\n## Resume Body\n"
    if marker in body:
        return strip_frontmatter_block(body.split(marker, 1)[1].strip())
    if body.startswith("## Resume Body\n"):
        return strip_frontmatter_block(body.split("## Resume Body\n", 1)[1].strip())
    return strip_frontmatter_block(body.strip())

def normalize_for_teal(text: str) -> str:
    text = text.strip()

    # Defensive cleanup for assembled-v2 files that accidentally embed
    # resume-version metadata at the top without a closing frontmatter marker.
    if text.startswith("---"):
        marker = "<!-- Generated by Career System"
        if marker in text:
            text = text.split(marker, 1)[1]
            text = marker + text
        elif "## Professional Summary" in text:
            text = "## Professional Summary" + text.split("## Professional Summary", 1)[1]

    text = re.sub(r"^# .+?Assembled Resume v1\s*\n+", "", text)
    text = text.replace("## Core Skills", "## Skills")
    if "\n## Assembly Notes" in text:
        text = text.split("\n## Assembly Notes", 1)[0].rstrip()
